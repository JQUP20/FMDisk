...
/*****************************************************************
    OPENCOMM_DesExpandEncKey	: Expand Des Enc Key ?©å?des??å¯?å¯???
    Return value:
        0         : Success
        other     : failed
    Parameters:
        pbDesKey        : ?©å?????DESå¯???8å­???)       input
        ulDesKeyLen     : ?©å?????DESå¯??¥é?¿åº¦          input
        pbDesEncKey     : ?©å?????DES??å¯?å¯???128å­???)  output
        *ulDesEncKeyLen : ?©å?????DES??å¯?å¯??¥é?¿åº¦       output
*****************************************************************/
unsigned long OPENCOMM_DesExpandEncKey(
        unsigned char *pbDesKey,
        unsigned long  ulDesKeyLen,
...
/*****************************************************************
    OPENCOMM_DesExpandDecKey	: Expand Des Dec Key ?©å?desè§£å?å¯???
    Return value:
        0       : Success
        other   : failed
    Parameters:
        pbDesKey        : ?©å?????DESå¯???8å­???)      input
        ulDesKeyLen     : ?©å?????DESå¯??¥é?¿åº¦         input
        pbDesDecKey     : ?©å?????DESè§£å?å¯???128å­???) output
        *ulDesDecKeyLen : ?©å?????DESè§£å?å¯??¥é?¿åº¦      output
*****************************************************************/
unsigned long OPENCOMM_DesExpandDecKey(
        unsigned char *pbDesKey,
        unsigned long  ulDesKeyLen,
...
/****************************************************************
    OPENCOMM_DesEncRaw		: Desç®?æ³???å¯?å°??´å??????8å­???
    Return value:
        0       : Success
        other   : failed
    Parameters:
        pbDesEncKey    : DES??å¯?å¯???   input
        ulDesEncKeyLen : DES??å¯?å¯??¥é?¿åº¦ input
        pbInData       : å¾???å¯???????    input
        ulInDataLen    : å¾???å¯????????¿åº¦ input
        pbOutData      : ??å¯?????å¯???    output
        *ulOutDataLen  : ??å¯?????å¯????¿åº¦ output
*****************************************************************/
unsigned long OPENCOMM_DesEncRaw(
        unsigned char *pbDesEncKey,
        unsigned long  ulDesEncKeyLen,
...
/*****************************************************************
    OPENCOMM_DesDecRaw : Desç®?æ³?è§£å?å°??´å??å¯???8å­???
    Return value:
        0     : Success
        other : failed
    Parameters:
        pbDesDecKey    : DESè§£å?å¯???    input
        ulDesDecKeyLen : DESè§£å?å¯??¥é?¿åº¦  input
        pbInData       : å¾?è§£å???å¯???     input
        ulInDataLen    : å¾?è§£å???å¯????¿åº¦  input
        pbOutData      : è§£å?????????     output
        *ulOutDataLen  : è§£å??????????¿åº¦  output
*****************************************************************/
unsigned long OPENCOMM_DesDecRaw(
        unsigned char *pbDesDecKey,
        unsigned long  ulDesDecKeyLen,
           ...
           int           *pOutDataLen)
{
    ...
    if ( (int)padChar<=0 || (int)padChar>8) //å¼?å¸¸å???
    {
